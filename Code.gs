...
const WALLET_ADDRESS2 = PropertiesService.getScriptProperties().getProperty('WALLET_ADDRESS2');
// Parse comma-separated filtered tokens (e.g. "SCAM, BAD")
const FILTERED_TOKENS_STR = PropertiesService.getScriptProperties().getProperty('FILTERED_TOKENS');
const FILTERED_TOKENS = (FILTERED_TOKENS_STR || '').toUpperCase().split(',').map(s => s.trim()).filter(s => s);
...
function recordPortfolioSnapshot() {
  // Create array of wallets, filtering out empty ones
  const wallets = [WALLET_ADDRESS, WALLET_ADDRESS2].filter(w => w && w.trim() !== '');

  if (!MORALIS_API_KEY || wallets.length === 0 || MORALIS_API_KEY.includes('YOUR_')) {
    SpreadsheetApp.getUi().alert('Please setup "MORALIS_API_KEY" and at least one "WALLET_ADDRESS" in Script Properties.');
    return;
  }
  ...
  try {
    ...
    wallets.forEach(walletAddr => {
      ...
      chains.forEach(chain => {
        Logger.log(`Fetching ${chain.name} for ${walletAddr}...`);
        const chainAssets = fetchEvmChainAssets(walletAddr, chain.id, chain.name);
        assets.push(...chainAssets);
      });

      // 2. Fetch Hyperliquid Assets (HYPE & USDC)
      const hlAssets = fetchHyperliquidAssets(walletAddr);
...
function fetchEvmChainAssets(address, chainId, chainName) {
  ...
  if (Array.isArray(tokens)) {
    tokens.forEach(t => {
      ...
      if (FILTERED_TOKENS.includes(t.symbol.toUpperCase())) return;
...
function fetchHyperEvmErc20s(address) {
  ...
  tokens.forEach(token => {
    // Filter out unwanted tokens
    if (FILTERED_TOKENS.includes(token.symbol.toUpperCase())) return;
